...
window.skel = __webpack_require__(9);
...
(function ($) {
	...
	$.fn.poptrox = function (options) {
		...
		if (settings.usePopupDefaultStyling) {
			...
			$popup.css('background', settings.popupBackgroundColor).css('color', settings.popupTextColor).css('padding', settings.popupPadding + 'px');

			if ($caption.length > 0) {

				$popup.css('padding-bottom', settings.popupCaptionHeight + 'px');

				$caption.css('position', 'absolute').css('left', '0').css('bottom', '0').css('width', '100%').css('text-align', 'center').css('height', settings.popupCaptionHeight + 'px').css('line-height', settings.popupCaptionHeight + 'px');

				if (settings.popupCaptionTextSize) $caption.css('font-size', popupCaptionTextSize);
			}

			if ($closer.length > 0) $closer.html(settings.popupCloserText).css('font-size', settings.popupCloserTextSize).css('background', settings.popupCloserBackgroundColor).css('color', settings.popupCloserTextColor).css('display', 'block').css('width', '40px').css('height', '40px').css('line-height', '40px').css('text-align', 'center').css('position', 'absolute').css('text-decoration', 'none').css('outline', '0').css('top', '0').css('right', '-40px');

			if ($loader.length > 0) {

				$loader.html('').css('position', 'relative').css('font-size', settings.popupLoaderTextSize).on('startSpinning', function (e) {

					var x = $('<div>' + settings.popupLoaderText + '</div>');

					x.css('height', Math.floor(settings.popupHeight / 2) + 'px').css('overflow', 'hidden').css('line-height', Math.floor(settings.popupHeight / 2) + 'px').css('text-align', 'center').css('margin-top', Math.floor(($popup.height() - x.height() + ($caption.length > 0 ? $caption.height() : 0)) / 2)).css('color', settings.popupTextColor ? settings.popupTextColor : '').on('xfin', function () {
		...
		$popup.css('display', 'inline-block').css('vertical-align', 'middle').css('position', 'relative').css('z-index', 1).css('cursor', 'auto').appendTo($overlay).hide().on('poptrox_next', function () {
			...
			if (x.type == 'ajax') $.get(x.src, function (data) {
			...
			if (x.type != 'image') {
				...
				if (xwidth.slice(-1) == '%') xwidth = parseInt(xwidth.substring(0, xwidth.length - 1)) / 100.00 * $window.width();

				if (xheight.slice(-1) == '%') xheight = parseInt(xheight.substring(0, xheight.length - 1)) / 100.00 * $window.height();

				$x.css('position', 'relative').css('outline', '0').css('z-index', settings.baseZIndex + 100).width(xwidth).height(xheight);
			}
			...
			if (settings.popupIsFixed) {
				...
				$x.load(function () {
					...
					$pic.css('text-indent', 0).hide().fadeIn(settings.fadeSpeed, function () {
						isLocked = false;
					});
					navPos = index;
				...
				$x.load(function () {
					...
					if (nw == $popup.data('width') && nh == $popup.data('height')) f();else $popup.animate({ width: nw, height: nh }, settings.popupSpeed, 'swing', f);
				});
			}
		...
		} else $popup.on('click', function (e) {
			e.stopPropagation();
		});
		...
		$this.find(settings.selector).each(function (index) {
			...
			// Function?
				...
				// Selector?
						...
						if ('attribute' in settings.caption) c = s.attr(settings.caption.attribute);else {
			...
			if (window.location.protocol == 'file:' && x.src.match(/^\/\//)) x.src = 'http:' + x.src;

			queue.push(x);

			i.attr('title', '');

			a.attr('href', '').css('outline', 0).on('click', function (e) {
